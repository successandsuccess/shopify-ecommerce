...
function attachChangeEventToColorInput() {
  if ($('.card-product-custom .variant-options .color-input').length > 0) {
...
class FacetFiltersForm extends HTMLElement {
  ...
  onSubmitHandler(event) {
    ...
    if (event.srcElement.className == 'mobile-facets__checkbox') {
      ...
      setTimeout(slickReinit, 1000);
      setTimeout(attachChangeEventToColorInput, 2100);
    } else {
      const forms = [];
...
class FacetRemove extends HTMLElement {
  ...
  closeFilter(event) {
    ...
    // Delay the initial check for the element by 1000 milliseconds (1 second)
    setTimeout(slickReinit, 1000);
    setTimeout(attachChangeEventToColorInput, 2100);
  }
}
